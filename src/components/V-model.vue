...
<div>
  ...
  <label>
    <span>
      <input type="checkbox" v-model="checked">
    </span>用label包住input 不需要再在input上加id属性和对应的label上加for属性
  </label>
  <br>
  不良嗜好：{{hobby}}
  <br>
  ...
  <select v-model="want" name="want2" id="want2">
    ...
    <option v-for="item in want2" :value="item.value" :key="item.value"> {{ item.text }} </option>
  </select>
  <br>
  你想要{{hope}}
  <br>
  ...
  <form @submit.prevent="onSubmit">
    ...
    <h3>equals Input &lt;MyInput :value="user.username" @input="user.username = $event"&gt;</h3>
